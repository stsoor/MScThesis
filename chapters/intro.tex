...
\begin{table}[H]
	...
	\begin{tabular}{ | m{0.25\textwidth} | m{0.65\textwidth} | }
		...
		\hline \hline %\hline
		\emph{Hamis pozitív tartalmazás} & Egy adott $v$ hipercsúcstól elvárjuk, hogy a neki megfelelő pont az ábrán ne szerepeljen egyetlen olyan hiperélt reprezentáló görbében sem, amely hiperél nem tartalmazza őt. \\
		\hline
		\emph{Hamis negatív tartalmazás} & Egy adott $v$ hipercsúcstól elvárjuk, hogy a neki megfelelő pont az ábrán szerepeljen minden olyan hiperélt reprezentáló görbében, amely hiperél tartalmazza őt. \\
		\hline
	\end{tabular}
	% \label{tab:example-1}
\end{table}

A hamis pozitív tartalmazást úgy vizsgálom, hogy minden $E_i$ hiperél minden $Seg_j \in E_i$ szegmense esetén egy hibásan tartalmazott csúcs $dist(v, Seg_j) / (|\{v \in V | v \notin Seg_j\}|*min(width_{S_j} / 2, height_{S_j} / 2)*|Seg|)$ mennyiséggel járul hozzá a metrika értékéhez, ahol $dist(v, Seg_j)$ jelöli egy belső csúcs távolságát a poligon széleitől, $width_{S_j}$ és $height_{S_j}$ a szegmenst ábrázoló poligon befoglaló négyzetének megfelelő paraméterei (pixelben), $Seg$ pedig az összes szegmensek halmaza. A nevező tagjai normalizálják a szegmenseken és nem tartalmazott csúcsokon való iterálást, illetve a távolságot a szélektől. Mivel a konvex burok egy belső pontjának a szélektől való legnagyobb lehetséges távolságának meghatározása nem triviális, ezért azt a befoglaló négyzettel közelítjük, amely felülről becsüli azt. Ez természetesen azt jelenti, hogy nem feltétlenül érhető el az (legrosszabb) $1$ veszteségfüggvény-érték, viszont az továbbra is a $[0, 1]$ intervallumból fog kikerülni.

Hamis negatív tartalmazás a mi esetünkben nem fordulhat elő, hiszen a csúcsok konvex burkait használjuk, amely szükségszerűen tartalmazza az őt kifeszítő pontokat.
...
\begin{longtable}{| p{0.25\textwidth} | p{0.65\textwidth} |}
		...
		\textbf{Elnevezés} & \textbf{Leírás} \\
		\hline \hline %\hline
		\emph{Szegmensek száma} &  \\
		\hline
		\emph{Szabályosság} & \\
		\hline
		\emph{Fragmentálatlanság} & \\
		\hline
		\emph{Minimum távolság fenntartása} & \\
		\hline
		\emph{Területarányosság} &  \\
		\hline
		\emph{Univerzum arányosság} & \\
		\hline
		\emph{Élmetszet} &\\
		\hline
		\emph{Legközelebbi szomszéd élszomszéd} & \\
		\hline
...
Az előző fejezetekben a megoldási módszert részleteiben is áttekintettük, azonban a jobb átláthatóság érdekében úgy gondolom, hasznos egyben is látni, hogy hogy áll össze egésszé.

% TODO bemenetek, költségfüggvény, heurisztikák

\section{Mérések és következtetések}

% különböző méretű problémák (élek, csúcsok, mindkettő változtatása)
% tanulás sebessége, változás méretarányosan
% helyettesítő heurisztikák összehasonlítása (tulajdonságok, kétfa
